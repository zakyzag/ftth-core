...
async function loadPeta() {
  ...
  petaMap.on('load', async () => {
    ...
    petaMap.on('click', (e) => {
      ...
      if (modeGambarGaris) { handleKlikGaris(e.lngLat); }
      else { tampilFormTambahODP(e.lngLat); }
    });
  ...
  const formHtml = `
    <div style="background:#0a1628;border:1px solid rgba(0,255,136,0.3);border-radius:10px;padding:16px;min-width:220px;font-family:sans-serif;color:white;">
      ...
      <div style="display:flex;gap:8px;">
        ...
        <button onclick="document.querySelectorAll('.mapboxgl-popup').forEach(p=>p.remove())" style="flex:1;background:rgba(255,50,50,0.1);border:1px solid rgba(255,50,50,0.3);color:#ff6666;padding:8px;border-radius:6px;cursor:pointer;font-size:12px;">BATAL</button>
      </div>
    </div>`;
  new mapboxgl.Popup({ className: 'odp-popup', maxWidth: '280px' }).setLngLat(lnglat).setHTML(formHtml).addTo(petaMap);
}
...
function handleKlikGaris(lnglat) {
  if (!titikGarisSementara) {
    ...
    new mapboxgl.Popup({ className: 'odp-popup', maxWidth: '280px' }).setLngLat(titikAkhir).setHTML(formHtml).addTo(petaMap);
    titikGarisSementara = null;
  }
}
